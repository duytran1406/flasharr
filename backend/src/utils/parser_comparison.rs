...
mod tests {
    ...
    fn compare_parsers() {
        ...
        let file_path =
            "/Users/blavkbeav/Documents/Workspace/Flasharr/Flasharr/debug_log/dataset_1.json";
        ...
        writeln!(
            file,
            "{:<100} | {:<60} | {:<60} | {}",
            "Filename", "Old (FilenameParser)", "New (SmartTokenizer)", "Verdict"
        )
        .unwrap();
        ...
        for filename in samples {
            ...
            writeln!(
                file,
                "{:<100} | {:<60} | {:<60} | {}",
                filename, old_map, new_map, verdict
            )
            .unwrap();
            ...
            let old_is_movie = old.episode.is_none()
                && old.season.is_none()
                && (old.year.is_some() || old.title.to_lowercase().contains("movie"));
            if old_is_movie {
                old_identified_movies += 1;
            }

            if new.media_type == MediaType::Movie {
                new_identified_movies += 1;
            }

            if old.episode.is_some() {
                old_ep_found += 1;
            }
            if new.episode.is_some() {
                new_ep_found += 1;
            }
        }
        ...
        writeln!(
            file,
            "{:<25} | {:<15}",
            "Old (Inferred)", old_identified_movies
        )
        .unwrap();
        writeln!(
            file,
            "{:<25} | {:<15}",
            "New (Explicit)", new_identified_movies
        )
        .unwrap();

        writeln!(
            file,
            "\nEpisodes Found: Old={}, New={}",
            old_ep_found, new_ep_found
        )
        .unwrap();
