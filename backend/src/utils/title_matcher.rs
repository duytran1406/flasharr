...
pub fn get_franchise_conflicts() -> &'static Vec<(&'static str, Vec<&'static str>)> {
    FRANCHISE_CONFLICTS.get_or_init(|| {
        vec![
            ...
            (
                "terminator",
                vec!["genisys", "dark fate", "salvation", "rise of the machines"],
            ),
            ("matrix", vec!["resurrections", "reloaded", "revolutions"]),
            (
                "star wars",
                vec!["rogue one", "solo", "mandalorian", "andor", "clone wars"],
            ),
            ...
            (
                "john wick",
                vec!["chapter 2", "chapter 3", "chapter 4", "parabellum"],
            ),
            (
                "spider-man",
                vec![
            ...
            (
                "batman",
                vec!["begins", "dark knight", "rises", "forever", "robin"],
            ),
            (
                "transformers",
                vec![
            ...
            (
                "pirates",
                vec!["dead man", "world's end", "stranger tides", "dead men"],
            ),
            (
                "harry potter",
                vec![
                    "chamber", "prisoner", "goblet", "phoenix", "prince", "hallows",
                ],
            ),
            (
                "lord of the rings",
                vec!["two towers", "return of the king", "fellowship"],
            ),
            ("hobbit", vec!["unexpected", "desolation", "five armies"]),
            (
                "hunger games",
                vec!["catching fire", "mockingjay", "ballad"],
            ),
...
pub fn extract_core_title(text: &str) -> String {
    ...
    name = FilenameParser::quality_pattern()
        .replace_all(&name, " ")
        .to_string();
    name = FilenameParser::year_pattern()
        .replace_all(&name, " ")
        .to_string();
    name = EXT_RE
        .get_or_init(|| Regex::new(r"(?i)\.(mkv|mp4|avi|m4v|wmv|flv|webm|ts|m2ts)$").unwrap())
        .replace_all(&name, "")
        .to_string();
    ...
    name = POSS_RE
        .get_or_init(|| Regex::new(r"(?i)['’]s\b").unwrap())
        .replace_all(&name, "s ")
        .to_string();
    ...
    name = TV_NUM_RE
        .get_or_init(|| Regex::new(r"(?i)^(?:tap|ep|episode|chapter)?\s?\d{1,3}[._\s-]").unwrap())
        .replace(&name, " ")
        .to_string();
...
pub fn get_title_keywords(title: &str) -> HashSet<String> {
    ...
    let stop_words: HashSet<&str> = [
        ...
        "from", "as", "is", "was", "are", "were", "be", "been", "being", "have", "has", "had", "i",
        "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix", "x", "va", // Vietnamese "and"
    ]
    .iter()
    .cloned()
    .collect();
...
fn _match_against_title(search_title: &str, filename: &str) -> SimilarityResult {
    if is_different_franchise_entry(search_title, filename) {
        return SimilarityResult {
            score: 0.0,
            is_valid: false,
            match_type: "franchise_conflict".to_string(),
        };
    }
    ...
    if search_words.is_empty() {
        return SimilarityResult {
            score: 0.1,
            is_valid: false,
            match_type: "no_keywords".to_string(),
        };
    }

    let n_file_words: HashSet<String> =
        file_words.iter().map(|w| normalize_vietnamese(w)).collect();
    ...
    if !missing_words.is_empty() {
        ...
        return SimilarityResult {
            ...
            is_valid: false,
            match_type: "missing_keywords".to_string(),
        };
    }

    let extra_words = if file_words.len() > common_count {
        file_words.len() - common_count
    } else {
        0
    };
    ...
    SimilarityResult {
        score,
        is_valid: true,
        match_type: "all_keywords".to_string(),
    }
}
...
pub fn is_vietnamese_title(text: &str) -> bool {
    ...
    text.to_lowercase()
        .chars()
        .any(|c| vietnamese_specific.contains(c))
}
...
pub fn normalize_vietnamese(text: &str) -> String {
    ...
    let mappings = [
        (
            "\u{00E0}\u{00E1}\u{1EA3}\u{00E3}\u{1EA1}\u{0103}\u{1EB1}\u{1EB3}\u{1EB5}\u{1EB7}\u{00E2}\u{1EA7}\u{1EA5}\u{1EA9}\u{1EAB}\u{1EAD}",
            'a',
        ),
        (
            "\u{00E8}\u{00E9}\u{1EBB}\u{1EBD}\u{1EB9}\u{00EA}\u{1EC1}\u{1EBF}\u{1EC3}\u{1EC5}\u{1EC7}",
            'e',
        ),
        ("\u{00EC}\u{00ED}\u{1EC9}\u{0129}\u{1ECB}", 'i'),
        (
            "\u{00F2}\u{00F3}\u{1ECF}\u{00F5}\u{1ECD}\u{00F4}\u{1ED3}\u{1ED5}\u{1ED7}\u{1ED9}\u{01A1}\u{1EDD}\u{1EDB}\u{1EDF}\u{1EE1}\u{1EE3}",
            'o',
        ),
        (
            "\u{00F9}\u{00FA}\u{1EE7}\u{0169}\u{1EE5}\u{01B0}\u{1EEB}\u{1EED}\u{1EEF}\u{1EF1}",
            'u',
        ),
