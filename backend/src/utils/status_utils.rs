...
impl StatusCounts {
    ...
    pub fn from_db_counts(counts: HashMap<String, usize>) -> Self {
        let downloading =
            *counts.get("DOWNLOADING").unwrap_or(&0) + *counts.get("STARTING").unwrap_or(&0);
        let queued = *counts.get("QUEUED").unwrap_or(&0) + *counts.get("WAITING").unwrap_or(&0);
...
mod tests {
    ...
    fn test_normalize_status_filter() {
        assert_eq!(
            normalize_status_filter("downloading"),
            vec!["DOWNLOADING", "STARTING"]
        );
