...
use crate::downloader::{DownloadState, DownloadTask};
...
pub fn aggregate_batch_state(tasks: &[DownloadTask]) -> DownloadState {
    ...
    let failed = tasks
        .iter()
        .any(|t| matches!(t.state, DownloadState::Failed));
    let paused = tasks
        .iter()
        .any(|t| matches!(t.state, DownloadState::Paused));
    let queued = tasks
        .iter()
        .any(|t| matches!(t.state, DownloadState::Queued | DownloadState::Waiting));
    let all_completed = tasks
        .iter()
        .all(|t| matches!(t.state, DownloadState::Completed));
...
mod tests {
    ...
    fn make_task(state: DownloadState, size: u64, progress: f32) -> DownloadTask {
        DownloadTask {
            ...
            size,
            progress,
            speed: if matches!(state, DownloadState::Downloading) {
                1000.0
            } else {
                0.0
            },
