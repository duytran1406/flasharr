...
use super::base::{AccountStatus, FileInfo, HostHandler, ResolvedUrl};
    ...
    async fn can_login(&self) -> Result<(), std::time::Duration> {
        ...
        if let Some(last) = *last_attempt {
            ...
            if elapsed < required_wait {
                ...
                return Err(std::time::Duration::from_secs(
...
impl FshareHandler {
    ...
    async fn validate_session(&self, session: &SessionData) -> anyhow::Result<()> {
        ...
        match api2_result {
            ...
            Ok(resp) => {
                tracing::debug!(
                    "[FSHARE-API2] Session invalid (status: {}), trying primary API",
                    resp.status()
                );
            }
            Err(e) => {
                tracing::debug!(
                    "[FSHARE-API2] Validation failed ({}), trying primary API",
                    e
                );
            }
        }
    ...
    async fn perform_login_protected(&self) -> anyhow::Result<SessionData> {
        ...
        match self.rate_limiter.can_login().await {
            ...
            Err(wait_duration) => {
                tracing::warn!(
                    "[FSHARE] Rate limited, waiting {}s before login...",
        ...
        // Step 3: Double-check - another thread may have logged in or validated while we waited
        {
            ...
            if let Some(ref data) = *session {
                if !data.needs_validation() {
                    tracing::info!(
                        "[FSHARE] Another thread successfully validated or established session"
                    );
                    return Ok(data.clone());
                }
                ...
                if self.validate_session(&session_to_validate).await.is_ok() {
                    tracing::info!(
                        "[FSHARE] Session is still valid (checked inside lock), reusing"
                    );
                ...
                match self.validate_session(&saved_session).await {
                    Ok(_) => {
                        ...
                        *session = Some(validated_session.clone());
                        tracing::info!(
                            "[FSHARE] Successfully restored and validated session from database"
                        );
        ...
        tracing::info!(
            "[FSHARE] No valid session found in memory or database, performing fresh login"
        );
    ...
    async fn perform_login(&self) -> anyhow::Result<SessionData> {
        ...
        if !self.config.prefer_api2 {
            ...
            match self.perform_api_login(&email, &password).await {
                ...
                Err(e) => {
                    ...
                    {
                        tracing::warn!(
                            "[FSHARE] Primary API unreachable ({}), trying api.fshare.vn",
                            err_str
                        );
                    } else {
        ...
        match self.perform_api2_login(&email, &password).await {
            ...
            Err(e) => {
                ...
                tracing::warn!(
                    "[FSHARE] Secondary API login failed ({}), falling back to web login",
                    e
                );
            }
        }
        ...
        match self.perform_web_login(&email, &password).await {
            ...
            Err(web_err) => {
                ...
                Err(anyhow::anyhow!(
                    "All login methods failed. Primary API: unreachable, Secondary API: unreachable, Web: {}",
                    web_err
                ))
            }
        }
    ...
    async fn perform_api_login(&self, email: &str, password: &str) -> anyhow::Result<SessionData> {
        ...
        if let Some(ref db) = self.db {
            ...
            tracing::debug!(
                "[FSHARE] Cached rank '{}' and valid_until '{}' to DB",
                rank,
                valid_until
            );
        }
    ...
    async fn perform_api2_login(&self, email: &str, password: &str) -> anyhow::Result<SessionData> {
        ...
        let resp = if status == reqwest::StatusCode::MOVED_PERMANENTLY
        ...
        tracing::info!(
            "[FSHARE-API2] Login response body: {}",
            &body_text[..body_text.len().min(500)]
        );

        let data: Value = serde_json::from_str(&body_text).map_err(|e| {
            anyhow::anyhow!(
                "API2 login: invalid JSON response ({}): {}",
                e,
        ...
        if let Some(ref db) = self.db {
            ...
            if let Ok(status) = self.fetch_account_status_for_session(&new_session).await {
                ...
                let _ = db.save_setting("fshare_valid_until", &valid_until.to_string());
                tracing::info!(
                    "[FSHARE-API2] Verified account rank: {}, valid_until: {}",
                    rank,
                    valid_until
                );
            }
    ...
    async fn perform_web_login(
        ...
        let csrf_token = Self::extract_csrf_token(&html).ok_or_else(|| {
            anyhow::anyhow!("[FSHARE-WEB] Could not extract CSRF token from login page")
        })?;

        tracing::debug!(
            "[FSHARE-WEB] Got CSRF token: {}...",
            &csrf_token[..20.min(csrf_token.len())]
        );
        ...
        tracing::info!(
            "[FSHARE-WEB] Login POST status: {} | Cookies: {}",
            status,
            if all_cookies.len() > 60 {
                format!("{}...", &all_cookies[..60])
            } else {
                all_cookies.clone()
            }
        );
        ...
        if status == reqwest::StatusCode::FOUND || status == reqwest::StatusCode::MOVED_PERMANENTLY
        {
            ...
            if body.contains("LoginForm[email]") || body.contains("form-signup") {
                return Err(anyhow::anyhow!(
                    "[FSHARE-WEB] Login failed — invalid credentials"
                ));
            }
            tracing::info!("[FSHARE-WEB] Web login appears successful (200 with no login form)");
        } else {
            return Err(anyhow::anyhow!(
                "[FSHARE-WEB] Unexpected login response status: {}",
                status
            ));
        }
        ...
        match profile_resp {
            Ok(resp) => {
                ...
                if profile_status == reqwest::StatusCode::OK {
                    ...
                    let final_csrf = Self::extract_csrf_token(&profile_html).unwrap_or(csrf_token);
                    ...
                    return Err(anyhow::anyhow!(
                        "[FSHARE-WEB] Session invalid — /account/profile redirected to login"
                    ));
                }
            }
            Err(e) => {
                tracing::warn!(
                    "[FSHARE-WEB] Could not validate session via /account/profile: {}",
                    e
                );
            }
        }
    ...
    async fn validate_web_session(&self, session: &WebSessionData) -> anyhow::Result<()> {
        ...
        if resp.status() == reqwest::StatusCode::OK {
            ...
            Err(anyhow::anyhow!(
                "[FSHARE-WEB] Session expired (status: {})",
                resp.status()
            ))
        }
    }
    ...
    async fn resolve_download_url_web(&self, url: &str) -> anyhow::Result<ResolvedUrl> {
        ...
        if fcode.is_empty() {
            return Err(anyhow::anyhow!(
                "[FSHARE-WEB] Could not extract file code from URL"
            ));
        }
        ...
        tracing::info!(
            "[FSHARE-WEB] File code: {}, des: {}, POSTing form to /download/get",
            fcode,
        ...
        let (download_csrf, download_cookies) = match file_page_resp {
            Ok(resp) => {
                ...
                let csrf = Self::extract_csrf_token(&html)
                    .unwrap_or_else(|| web_session.csrf_token.clone());
                tracing::debug!(
                    "[FSHARE-WEB] Got file-page CSRF: {}...",
                    &csrf[..20.min(csrf.len())]
                );
                (csrf, merged)
            }
            Err(e) => {
                tracing::warn!(
                    "[FSHARE-WEB] Could not fetch file page for fresh CSRF ({}), using session CSRF",
                    e
                );
        ...
        tracing::debug!(
            "[FSHARE-WEB] Form body (sanitised): linkcode={}, des={}",
            fcode,
            des_token
        );
            ...
            || status == reqwest::StatusCode::TEMPORARY_REDIRECT
        {
            if let Some(location) = resp.headers().get(reqwest::header::LOCATION) {
                let direct_url = location
                    ...
                    .to_string();
                tracing::info!(
                    "[FSHARE-WEB] Got redirect to download URL: {}...",
                    &direct_url[..50.min(direct_url.len())]
                );
        ...
        if let Ok(data) = serde_json::from_str::<Value>(&resp_body) {
            tracing::debug!(
                "[FSHARE-WEB] JSON response keys: {:?}",
                data.as_object().map(|o| o.keys().collect::<Vec<_>>())
            );

            let direct_url = data["url"]
                .as_str()
            ...
            if let Some(url_str) = direct_url {
                if !url_str.is_empty() {
                    tracing::info!(
                        "[FSHARE-WEB] Got download URL from JSON: {}...",
                        &url_str[..50.min(url_str.len())]
                    );
                    return Ok(ResolvedUrl {
            ...
            tracing::warn!(
                "[FSHARE-WEB] JSON response but no download URL: {}",
        ...
        if let Some(pos) = resp_body.find("https://download") {
            if let Some(end) = resp_body[pos..]
                .find('"')
                .or_else(|| resp_body[pos..].find('\''))
            {
                let direct_url = resp_body[pos..pos + end].to_string();
                tracing::info!(
                    "[FSHARE-WEB] Extracted download URL from HTML: {}...",
                    &direct_url[..50.min(direct_url.len())]
                );
                return Ok(ResolvedUrl {
                    direct_url,
        ...
        tracing::error!(
            "[FSHARE-WEB] Could not extract download URL. Status: {}, Body preview: {}",
            status,
            &resp_body[..500.min(resp_body.len())]
        );
        Err(anyhow::anyhow!(
            "[FSHARE-WEB] No download URL found in /download/get response (status {})",
            status
        ))
    }
    ...
    async fn fetch_account_status_for_session(
        &self,
        session: &SessionData,
        ...
        let data: Value = match api2_result {
            Ok(resp) if resp.status() == 200 => match resp.json().await {
                Ok(d) => d,
                Err(_) => Value::Null,
            },
            _ => Value::Null,
        ...
        tracing::info!(
            "[FSHARE] Account status verified: email='{}', type='{}', level='{}', is_premium={}, valid_until={:?}",
            email,
            account_type,
            level,
            is_premium,
    ...
    async fn resolve_download_url(&self, url: &str) -> anyhow::Result<ResolvedUrl> {
        ...
        if url.contains("/dl/") || url.contains("download.fshare") {
            ...
            match self.client.head(url).send().await {
                ...
                Ok(resp) => {
                    tracing::info!(
                        "[FSHARE] Existing premium link expired (status: {}), will re-resolve",
                        resp.status()
                    );
            ...
            return Err(anyhow::anyhow!(
                "Premium link expired, use original_url to re-resolve"
            ));
        }
        ...
        let session = self.ensure_valid_session().await?;
        tracing::info!(
            "[FSHARE] Session ready, session_id: {}...",
        ...
        tracing::info!(
            "[FSHARE] Request payload (sanitized): url={}, password=''",
            url
        );
        ...
        match api2_result {
            Ok(resp) => {
                    ...
                    || status == reqwest::StatusCode::TEMPORARY_REDIRECT
                {
                    ...
                    {
                        tracing::info!(
                            "[FSHARE-API2] Following download redirect to: {}",
                            location
                        );
                        match self
                            .api2_client
                ...
                if status.is_success() {
                    ...
                    match serde_json::from_str::<Value>(&body_text) {
                        Ok(data) => {
                            if let Some(location) = data["location"].as_str() {
                                if !location.is_empty() {
                                    ...
                                    tracing::info!(
                                        "[FSHARE-API2] Successfully resolved download URL"
                                    );
                                    return Ok(ResolvedUrl {
                        ...
                        Err(e) => {
                            tracing::warn!(
                                "[FSHARE-API2] Failed to parse JSON ({}), body: {}",
                                e,
                                &body_text[..body_text.len().min(200)]
                            );
                        }
                ...
                tracing::warn!(
                    "[FSHARE-API2] Failed (status {}), trying primary API",
                    status
                );
            }
            Err(e) => {
                ...
                if err_str.contains("timed out") || err_str.contains("connect") {
                    tracing::warn!(
                        "[FSHARE-API2] Unreachable ({}), trying primary API",
                        err_str
                    );
                } else {
        ...
        let api_result = self
            .client
            .post("https://download.fsharegroup.site/int/get")
        ...
        let resp = match api_result {
            ...
            Err(e) => {
                ...
                if err_str.contains("timed out") || err_str.contains("connect") {
                    tracing::warn!(
                        "[FSHARE] Primary API /int/get unreachable ({}), falling back to web",
                        err_str
                    );
        ...
        let direct_url = data["location"]
            .as_str()
            .or_else(|| data["url"].as_str())
            .ok_or_else(|| {
                tracing::error!(
                    "[FSHARE] No download URL found in response keys. Available keys: {:?}",
                    data.as_object().map(|o| o.keys().collect::<Vec<_>>())
                );
                ...
                anyhow::anyhow!(
                    "No download URL in response. Response: {}",
                    serde_json::to_string(&data).unwrap_or_default()
                )
            })?
            .to_string();
