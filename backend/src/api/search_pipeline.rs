...
pub const MEDIA_EXTENSIONS: &[&str] = &[
    "mkv", "mp4", "avi", "mov", "wmv", "flv", "webm", "m4v", "mpg", "mpeg", "m2ts", "vob", "3gp",
    "ogv", "divx", "rmvb", "rar", "zip", "7z", // archives that often contain media
];
...
impl SearchPipeline {
    ...
    pub async fn execute_fshare_search(
        client: &Client,
        query: &str,
        limit: usize,
    ) -> Vec<RawFshareResult> {
            ...
            Ok(r) => {
                if let Ok(data) = r.json::<Value>().await {
                    if let Some(arr) = data["data"].as_array() {
                        info!(
                            "TimFshare search '{}' returned {} results",
                            query,
                            arr.len()
                        );
