...
use crate::utils::title_matcher::{
    calculate_unified_similarity, extract_core_title, get_title_keywords,
};
use crate::AppState;
use axum::{
    extract::{Query, State},
...
async fn enhanced_search(
    ...
    let resp = client
        ...
        .header(
            "Referer",
            format!(
                "https://timfshare.com/search?key={}",
    ...
    match resp {
        Ok(r) => {
            if let Ok(data) = r.json::<serde_json::Value>().await {
                let mut results: Vec<SearchResult> = data["data"]
                    .as_array()
                    .map(|arr| {
                        arr.iter()
                            .map(|item| {
                                ...
                                let fcode =
                                    result_url.split("/file/").last().unwrap_or("").to_string();
                                let size = item["size"].as_u64().unwrap_or(0);

                                let parsed = smart_parse(&original_filename);
                                let sim_res = calculate_unified_similarity(
                                    &params.q,
                                    &original_filename,
                                    &[],
                                );
                                ...
                                let matched_count =
                                    (keywords.len() as f32 * sim_res.score).round() as i32;
                                ...
                                SearchResult {
                                    ...
                                    episode_tag,
                                    is_series: parsed.media_type
                                        == crate::utils::smart_tokenizer::MediaType::TvShow,
                                    ...
                                    overview: None,
                                    media_type: if parsed.media_type
                                        == crate::utils::smart_tokenizer::MediaType::TvShow
                                    {
                                        Some("tv".to_string())
                                    } else {
                ...
                if params.enrich && enrichment_pool_size > 0 {
                    ...
                    let mut collection_lookup: std::collections::HashMap<String, Value> =
                            ...
                            if let Some(coll_results) = coll_data["results"].as_array() {
                                if let Some(first_collection) = coll_results.first() {
                                    if let Some(collection_id) = first_collection["id"].as_u64() {
                                        ...
                                        if let Ok(details_resp) =
                                            client.get(&details_url).send().await
                                        {
                                            if let Ok(details_data) =
                                                details_resp.json::<Value>().await
                                            {
                                                if let Some(parts) =
                                                    details_data["parts"].as_array()
                                                {
                                                    for part in parts {
                                                        if let Some(title) = part["title"].as_str()
                                                        {
                                                            let normalized = normalize_title(title);
                                                            collection_lookup.insert(
                                                                normalized.clone(),
                                                                part.clone(),
                                                            );
                                                        }
