...
use crate::db::Db;
use crate::downloader::{DownloadOrchestrator, DownloadState, DownloadTask};
...
impl DownloadService {
    ...
    pub async fn pause_task(&self, id: Uuid) -> FlasharrResult<DownloadTask> {
        self.orchestrator.task_manager().pause_task(id).await.ok_or(
    ...
    pub async fn retry_task(&self, id: Uuid) -> FlasharrResult<DownloadTask> {
        self.orchestrator.task_manager().retry_task(id).await.ok_or(
            FlasharrError::DownloadInvalidState {
                id,
    ...
    pub async fn pause_batch(&self, batch_id: &str) -> FlasharrResult<usize> {
        ...
        let pauseable_ids: Vec<Uuid> = tasks
            .iter()
            .filter(|t| {
                matches!(
                    t.state,
        ...
        for task in tasks {
            if pauseable_ids.contains(&task.id) {
                if let Some(paused_task) =
                    self.orchestrator.task_manager().pause_task(task.id).await
                {
    ...
    pub async fn resume_batch(&self, batch_id: &str) -> FlasharrResult<usize> {
        ...
        let resumable_ids: Vec<Uuid> = tasks
            .iter()
            .filter(|t| {
