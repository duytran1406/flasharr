...
async def create_sabnzbd_service(account_manager: Optional['AccountManager'] = None) -> SABnzbdEmulator:
    ...
    # Create download engine with settings from store
    ...
    from .services.sabnzbd import DownloadStatus

    def engine_to_sab_callback(task):
        if task.state == DownloadState.COMPLETED:
            sabnzbd.complete_item(task.id, status=DownloadStatus.COMPLETED)
        elif task.state in (DownloadState.FAILED, DownloadState.CANCELLED, DownloadState.OFFLINE):
            sabnzbd.complete_item(task.id, status=DownloadStatus.FAILED)

    engine.progress_callback = engine_to_sab_callback
    
    # Restore persisted state to engine
    await sabnzbd.restore_state()
